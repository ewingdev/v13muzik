...
module.exports = {
    name: "arat",
    description: "youtubeden şarkı açarsınız",
    category: "Music",
    aliases: ["arat"],
    usage: [`Şarkı bulmak için şarkı aratın`],
    args: true,
    Permissions: [],
    owner: false,
    player: false,
   ...
   execute: async (message, args, client) => { 
    ...
    const msg = await message.channel.send({embeds: [new MessageEmbed().setColor(client.embedColor).setDescription(`? ${query} şarkısını arayın lütfen bekleyin`)]})
    ...
    switch (s.loadType) {
        case "TRACK_LOADED":
            ...
            const embed = new MessageEmbed()
             .setDescription(`${emojiplaylist} **Sıraya eklendi** - [${s.tracks[0].title}](${s.tracks[0].uri}) \`${convertTime(s.tracks[0].duration, true)}\` • ${s.tracks[0].requester}`)
         ...
         case "SEARCH_RESULT":
             ...
             const results = s.tracks.slice(0, 5).map(x => `• ${index++} | [${x.title}](${x.uri}) \`${convertTime(x.duration)}\``)
                    ...
                    const searched = new MessageEmbed()
                        .setTitle("İstediğiniz parçayı seçin")
                        .setColor(client.embedColor)
            ...
            collector.on("collect", async (b) => {
                ...
                if(b.customId === "s_one") {
                    player.queue.add(s.tracks[0]);
                        ...
                        if(msg) await msg.edit({embeds: [search.setDescription(`${emojiplaylist} **Sıraya eklendi** - [${s.tracks[0].title}](${s.tracks[0].uri}) \`${convertTime(s.tracks[0].duration, true)}\` • ${s.tracks[0].requester}`)]})
                } else if(b.customId === "s_two") {
                    player.queue.add(s.tracks[1]);
                    if(player && player.state === "CONNECTED" && !player.playing && !player.paused && !player.queue.size) await player.play();

                    if(msg) await msg.edit({embeds: [search.setDescription(`${emojiplaylist} **Sıraya eklendi** - [${s.tracks[1].title}](${s.tracks[1].uri}) \`${convertTime(s.tracks[1].duration, true)}\` • ${s.tracks[1].requester}`)]})
            
                } else if(b.customId === "s_three") {
                    player.queue.add(s.tracks[2]);
                    if(player && player.state === "CONNECTED" && !player.playing && !player.paused && !player.queue.size) await player.play();

                    if(msg) await msg.edit({embeds: [search.setDescription(`${emojiplaylist} **Sıraya eklendi** - [${s.tracks[2].title}](${s.tracks[2].uri}) \`${convertTime(s.tracks[2].duration, true)}\` • ${s.tracks[2].requester}`)]})
            
                } else if(b.customId === "s_four") {
                    player.queue.add(s.tracks[3]);
                    if(player && player.state === "CONNECTED" && !player.playing && !player.paused && !player.queue.size) await player.play();

                    if(msg) await msg.edit({embeds: [search.setDescription(`${emojiplaylist} **Sıraya eklendi** - [${s.tracks[3].title}](${s.tracks[3].uri}) \`${convertTime(s.tracks[3].duration, true)}\` • ${s.tracks[3].requester}`)]})
            
                } else if(b.customId === "s_five") {
                    player.queue.add(s.tracks[4]);
                    if(player && player.state === "CONNECTED" && !player.playing && !player.paused && !player.queue.size) await player.play();

                    if(msg) await msg.edit({embeds: [search.setDescription(`${emojiplaylist} **Sıraya eklendi** - [${s.tracks[4].title}](${s.tracks[4].uri}) \`${convertTime(s.tracks[4].duration, true)}\` • ${s.tracks[4].requester}`)]})
