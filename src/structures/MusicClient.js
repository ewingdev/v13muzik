 ...
 */
  ...
  readdirSync("./src/slashCommands/").forEach((dir) => {
        ...
        for (const file of slashCommandFile) {
            ...
            if(!slashCommand.name) return console.error(`slashCommandNameError: ${slashCommand.split(".")[0]} adlı komutta hata aldım`);

            if(!slashCommand.description) return console.error(`slashCommandDescriptionError: ${slashCommand.split(".")[0]} komut açıklamasında bir hata söz konusu.`);

            this.slashCommands.set(slashCommand.name, slashCommand);
            this.logger.log(`Eğik Çizgi (Slash Commands) yüklendi: ${slashCommand.name}`, "cmd");
            data.push(slashCommand);
        }
     });
	  this.on("ready", async () => {
        await this.application.commands.set(data).then(() => this.logger.log(`Slash Komutlar yeniden kayıt edildi.`, "cmd")).catch((e) => console.log(e));
    });
	 }
		 connect() {
